...
int SolveMatrix2(long double qMatrix[ROWS][COLS], long double qVector[ROWS], int d){
  ...
  /* } */
   ...
   fprintf(pipe, "set xrange[100:400]\n");
   //fprintf(pipe, "set yrange[1e-4:0.01]\n");
   //fprintf(pipe, "unset key\n");
   //fprintf(pipe, "set logscale\n");
   fprintf(pipe, "set xlabel \"E_{0} [keV]\"\n");
   fprintf(pipe, "set ylabel \"{/Symbol c}^{2}/f\"\n");
   fprintf(pipe, "set label 'a=2.89x10' at 110,7.5\n");
   fprintf(pipe, "set label 'b=-2.75x10^{-1}' at 110,7.0\n");
   fprintf(pipe, "set label 'c=2.89x10^{-3}' at 110,6.5\n");
   fprintf(pipe, "set label 'd=-2.00x10^{-6}' at 110,6.0\n");
   fprintf(pipe, "plot 'Usmantest.dat' using 1:2 title '{/Symbol c}^{2}/f', %Le+%Le*x+%Le*x*x+%Le*x*x*x notitle lt 3\n",xVector[0],xVector[1],xVector[2],xVector[3], xVector[0],xVector[1],xVector[2],xVector[3]);
