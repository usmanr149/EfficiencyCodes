...
double error(){
  ...
  aerror=sqrt(min*inverse[0][0]/(count0-8));
  berror=sqrt(min*inverse[1][1]/(count0-8));
  cerror=sqrt(min*inverse[2][2]/(count0-8));
  gerror=sqrt(min*inverse[3][3]/(count0-8));
  derror=sqrt(min*inverse[4][4]/(count0-8));
  eerror=sqrt(min*inverse[5][5]/(count0-8));
  fierror=sqrt(min*inverse[6][6]/(count0-8));
  energyerror=sqrt(min*inverse[7][7]/(count0-8));
}
...
int SolveMatrix2(long double qMatrix[ROWS][COLS], long double qVector[ROWS], int d){
  int i=0,j=0, k=0;
   ...
   fprintf(pipe, "set label '{/Italic c}_{0}=3.55x10^{3}' at 150,800\n");
   fprintf(pipe, "set label '{/Italic c}_{1}=-4.40x10^{1}' at 150,720\n");
   fprintf(pipe, "set label '{/Italic c}_{2}=1.74x10^{-2}' at 150,640\n");
   fprintf(pipe, "set label '{/Italic c}_{3}=2.06x10^{-4}' at 150,560\n");
   fprintf(pipe, "set arrow from 160,0 to 160, %10.6Le+%10.6Le*160+%10.6Le*160*160+%10.6Le*160*160*160 nohead lc 7 lw 1\n", xVector[0], xVector[1], xVector[2], xVector[3]);
   fprintf(pipe, "set arrow from 220,0 to 220, %10.6Le+%10.6Le*220+%10.6Le*220*220+%10.6Le*220*220*220 nohead lc 7 lw 1\n", xVector[0], xVector[1], xVector[2], xVector[3]);
