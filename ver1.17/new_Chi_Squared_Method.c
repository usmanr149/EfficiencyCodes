...
//fprintf(pipe, "set data style lines\n");
 ...
 fprintf(pipe, "set yrange[5e-4:0.015]\n");
 fprintf(pipe, "set key top\n");
 fprintf(pipe, "set xtics (50,1000,5000)\n");
 fprintf(pipe, "set logscale\n");
 fprintf(pipe, "set format y '%%.0e'\n");
 fprintf(pipe, "set label '{/Symbol c}^{2}_{min.}/({/Italic n} -{/Symbol u})= %10.3Lf' at 200,1e-2\n", min/(count0 - dim));
...
int SolveMatrix2(long double qMatrix[ROWS][COLS], long double qVector[ROWS], int d){
  int i=0,j=0, k=0;
   ...
   fprintf(pipe, "set xrange[120:280]\n");
   fprintf(pipe, "set yrange[30:300]\n");
   fprintf(pipe, "set key bottom\n");
   ...
   fprintf(pipe, "set label '{/Italic c}_{0}=3.53x10^{3}' at 170,270\n");
   fprintf(pipe, "set label '{/Italic c}_{1}=-4.37x10' at 170,255\n");
   fprintf(pipe, "set label '{/Italic c}_{2}=-1.72x10^{-1}' at 170,240\n");
   fprintf(pipe, "set label '{/Italic c}_{3}=-2.03x10^{-5}' at 170,225\n");
   fprintf(pipe, "set arrow from 160,30 to 160, %10.6Le+%10.6Le*160+%10.6Le*160*160+%10.6Le*160*160*160 nohead lc 7 lw 1\n", xVector[0], xVector[1], xVector[2], xVector[3]);
   fprintf(pipe, "set arrow from 220,30 to 220, %10.6Le+%10.6Le*220+%10.6Le*220*220+%10.6Le*220*220*220 nohead lc 7 lw 1\n", xVector[0], xVector[1], xVector[2], xVector[3]);
...
int main(int argc, char *argv[]){
  ...
  long double sumx5_=0, sumx6_=0, fx3_=0;
	...
	for(i=0; i<8; i++){
	  for(j=0; j<8; j++){
	    Uncorrected[i][j] = min/(count0 - dim)*inverse[i][j];
	...
	for(i=0;i<8;i++){
	  for(j=0;j<8;j++){
	    ...
	    printf("%10.6Le\t", bMatrix[i][j]* min/(count0 - 8));
	  }
	  printf("\n");
	}
	...
	fprintf(pipe, "set xlabel \"Energy [keV]\"\n");
	fprintf(pipe, "set ylabel \"Absolute Efficiency\n");
	fprintf(pipe, "plot 'Usmantest_scaledeff.dat' every ::0::1 using 1:2:3 title '{/Arial=12 ^{60}Co}' with yerrorbars pointtype 3 ps 1 linecolor rgb 'red', 'Usmantest_scaledeff.dat' every ::2::12 using 1:2:3 title '{/Arial=12 ^{152}Eu}' with yerrorbars pointtype 13 ps 1 linecolor rgb 'blue', 'Usmantest_scaledeff.dat' every ::13::24 using 1:2:3 title '{/Arial=12 ^{56}Co}' with yerrorbars pointtype 9 ps 1 linecolor rgb 'green', 'Usmantest_scaledeff.dat' every ::25::30 using 1:2:3 title '{/Arial=12 ^{133}Ba}' with yerrorbars pointtype 11 ps 0.8 linecolor rgb 'violet', x > %Lf  ?  exp(%s) : 1/0 notitle lt 1 lc 1 lw 1, x > %Lf ? exp(%s+1.7138*sqrt(%s)) : 1/0 notitle lt 1 lc 7 lw 1, x > %Lf ? exp(%s-1.7138*sqrt(%s)) : 1/0 notitle lt 1 lc 7 lw 1, x<%Lf ? exp(%s) : 1/0 title '{/Arial=12 Best-fit}' lt 1 lc 1 lw 1, x<%Lf ? exp(%s+1.7138*sqrt(%s)) : 1/0 notitle lt 1 lc 7 lw 1, x <%Lf ? exp(%s-1.7138*sqrt(%s)) : 1/0 title '{/Arial=12 90%% confidence interval}' lt 1 lc 7 lw 1", powl(2.71828182845,energy0) - 5, High_energy, powl(2.71828182845,energy0)-5, High_energy, text2, powl(2.71828182845,energy0)-5, High_energy, text2, powl(2.71828182845,energy0)+5, Low_energy, powl(2.71828182845,energy0)+3, Low_energy, text1, powl(2.71828182845,energy0)+5, Low_energy, text1);
	printf("Press enter to continue...\n");
	close(pipe); 
	fflush(pipe);

	printf("plot 'Usmantest_scaledeff.dat' every ::0::1 using 1:2:3 title '{/Arial=12 ^{60}Co}' with yerrorbars pointtype 3 ps 1 linecolor rgb 'red', 'Usmantest_scaledeff.dat' every ::2::12 using 1:2:3 title '{/Arial=12 ^{152}Eu}' with yerrorbars pointtype 13 ps 1 linecolor rgb 'blue', 'Usmantest_scaledeff.dat' every ::13::24 using 1:2:3 title '{/Arial=12 ^{56}Co}' with yerrorbars pointtype 9 ps 1 linecolor rgb 'green', 'Usmantest_scaledeff.dat' every ::25::30 using 1:2:3 title '{/Arial=12 ^{133}Ba}' with yerrorbars pointtype 11 ps 0.8 linecolor rgb 'violet', x > %Lf  ?  exp(%s) : 1/0 notitle lt 1 lc 1 lw 1, x > %Lf ? exp(%s+1.7138*sqrt(%s)) : 1/0 notitle lt 1 lc 7 lw 1, x > %Lf ? exp(%s-1.7138*sqrt(%s)) : 1/0 notitle lt 1 lc 7 lw 1, x<%Lf ? exp(%s) : 1/0 title '{/Arial=12 Best-fit}' lt 1 lc 1 lw 1, x<%Lf ? exp(%s+1.7138*sqrt(%s)) : 1/0 notitle lt 1 lc 7 lw 1, x <%Lf ? exp(%s-1.7138*sqrt(%s)) : 1/0 title '{/Arial=12 90%% confidence interval}' lt 1 lc 7 lw 1", powl(2.71828182845,energy0), High_energy, powl(2.71828182845,energy0), High_energy, text2, powl(2.71828182845,energy0), High_energy, text2, powl(2.71828182845,energy0), Low_energy, powl(2.71828182845,energy0), Low_energy, text1, powl(2.71828182845,energy0), Low_energy, text1);
	getchar();
	...
	/* fprintf(pipe, "plot 'Usmantest_scaledeff.dat', exp(%s), exp(%s+sqrt(%s)), %s, exp(%s+sqrt(%s))", Higher_energy, Higher_energy, text1, Low_energy, Low_energy, text2 ); */
	...
	Sec_order_first_term(); //Is first term matrix multiplied by 0.5*\sigma^4? yes
	
	Sec_order_Sec_term();  //Is first term matrix multiplied by 0.5*\sigma^4? yes
	...
	printf("Above is the second order correction");
	getchar();
