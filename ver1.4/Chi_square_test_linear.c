...
/* elimination.                                             */
/* Chi square analysis                                      */
/* In this program x0 is varied from min_energy all the way */
/* to max for every Chi-squared fit.                        */
/************************************************************/
...
long double energy, energy0=0, min=10000000,record_energy=0,abest=0,bbest=0,cbest=0,gbest=0,alphabest=0,betabest=0;
	...
	while(fscanf(inp,"%Lf %Lf %Lf\n",&xi[count],&yi[count],&ei[count])!=EOF)
	  {
	    ...
	    count++;
	      ...
	      long double energy00=xi[0];

	      for(upper_limit=2;upper_limit<(count-3);upper_limit++)
		{
		  energy0=energy00;
		   /* printf("count = %d\n", count); */
		   /* printf("xi[count-1] = %Lf\n", powl(10,xi[count-1])); */
		   /* printf("energy0 = %Lf\n", powl(10,energy0)); */
		   /* getchar(); */
		  jug=0;
		  while(energy0<xi[count-1])
		    {
		      
		      energy0=energy00+0.05*jug; 
		      /* printf("energy0 = %Lf\n", powl(10,energy0)); */
		      /* printf("jug = %d\n", jug); */
		      /* printf("xi[upper_limit+1] = %Lf\n", powl(10,xi[upper_limit+1])); */
		      /* getchar(); */
...
int ChiSquared(){
  int i;
  long double chisquared=0,chisquaredl=0,chisquaredr=0,minnie;
  ...
  for(i=0;i<upper_limit;i++)
    {
      ...
      chisquaredl+=(yi[i]-(alpha + beta*xi[i] + g*xi[i]*xi[i]))*(yi[i]-(alpha + beta*xi[i] + g*xi[i]*xi[i]));
  ...
  for(i=upper_limit;i<count;i++)
    {
      chisquaredr+=(yi[i]-(a + b*xi[i] + c*xi[i]*xi[i]))*(yi[i]-(a + b*xi[i] + c*xi[i]*xi[i]));
      
    }

  chisquared=chisquaredr+chisquaredl;
...
//fprintf(pipe, "plot '%s' using 1:2\n",file_name);
 fprintf(pipe, "set xrange[0:4]\n");
 //fprintf(pipe, "set logscale\n");
 fprintf(pipe, "plot 'AbsEff.txt' using 1:2, x <9 ? %Lf+%Lf*x+%Lf*x*x : 1/0, x < 9 ? %Lf+%Lf*x+%Lf*x*x : 1/0\n",abest,bbest,cbest, alphabest,betabest,gbest);
 printf("Press enter to continue...\n");
close(pipe); 
fflush(pipe);
getchar();
