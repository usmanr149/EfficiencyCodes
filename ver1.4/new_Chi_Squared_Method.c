...
double determinant(long double bMatrix[ROWS][COLS], int k)  //long double eMatrix[ROWS][COLS],int dim)
{
  long double s=1,det=0,bh[ROWS][COLS];
  int i,j,m,n,c,l;
...
//printf("eMatrix[%d][%d] = %Lf\n", i,j,eMatrix[i][j]);
	     ...
	     /* printf("Press any button to continue..."); */
          ...
          det=det + s * (bMatrix[0][c] * determinant(bh,k-1));
          s=-1 * s;
       }
    }
  printf("det = %Lf\n", det);
...
double MatrixCoefficients(long double bMatrix[ROWS][COLS])
{
  int i,j;
