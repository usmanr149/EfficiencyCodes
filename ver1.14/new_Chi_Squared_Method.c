...
long double Solution_vector[10][10];
...
long double Sec_order_first_term(){
  ...
  for(a=0; a<8; a++){
    for(b=0; b<8; b++){
      First_Term[a][b] = 0.5*(min/(count0 - dim))*(min/(count0 - dim))*Multiplication_answer[a][b];
...
long double Sec_order_Sec_term(){
  ...
  for(e=0;e<8;e++){
    for(f=0;f<8;f++){
      Second_Term[e][f]=(min/(count0 - dim))*(min/(count0 - dim))*Multiplication_answer[e][f];
...
long double curvature(){
  ...
  Matrix_multiplication(Holder, 8, 8, F_prime, 8, 8);
  
  for(a=0;a<8; a++){
    for(b=0;b<8;b++){
      Hbar[a][b] = Multiplication_answer[a][b];
    }
  }

  for(a=0;a<8; a++){
    for(b=0;b<8;b++){
      Hbar_subtract = Hbar[a][b] -Identity[a][b];
    }
  }
...
int main(int argc, char *argv[]){
  ...
  long double sumx5_=0, sumx6_=0, fx3_=0;
	...
	Solution_vector[4][0] = dbest;
	Solution_vector[5][0] = ebest;
	Solution_vector[6][0] = fbest;
	Solution_vector[7][0] = energy0;
	...
	for(i=0; i<8; i++){
	  for(j=0; j<8; j++){
	    Final_Second_order_Correction[i][j] = inverse[i][j] + First_Term[i][j] + Second_Term[i][j];
	    printf("%10.6Lf \t", Final_Second_order_Correction[i][j]);
	  }
	  printf("\n");
	}
