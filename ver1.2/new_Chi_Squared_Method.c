...
double MatrixCoefficients(){
  ...
  //eMatrix = (int *)malloc(sizeof(long double)*100);  // allocate 100 long doubles
...
int formMatrix()
{
  ...
  SolveMatrix();
  ChiSquared();
  energy0=log10(powl (10,energy00)+20*jug);
  jug++;}while(energy0<log10(3000));

}

int ChiSquared(){
  int i;
  long double chisquared,chisquaredl,chisquaredr;

  //*chisquared = malloc( sizeof(*chisquared) );
